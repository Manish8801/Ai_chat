...
import { Chat, User } from "@/generated/prisma/client";
...
type Props = {
  ...
  chats: ChatGetPayload<{ include: { messages: true } }>[];
};
const ChatSidebar = ({ user, chats }: Props) => {
  const { activeChatId, setActiveChatId } = useChatStore();
  const [selectedChatId, setSelectedChatId] = useState<Chat["id"] | null>(null);
  ...
  const onDelete = (e: MouseEvent<HTMLDivElement>, chatId: Chat["id"]) => {
    e.preventDefault();
    e.stopPropagation();
